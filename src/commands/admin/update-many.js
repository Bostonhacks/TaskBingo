...
module.exports = {
        ...
        users.map(async (user) => {
            if (!user) {
                ...
                if (interaction.options.get('value').value) {
                  ...
                  interaction.followUp(`Unchecked ${userObj} task ${interaction.options.get('cell-number').value} (${bingoCount}/${user.card.length*2+2}).`);
                }
            }
    ...
    data: {
      ...
      options: [
        {
          name: 'users',
          description: 'Usernames of users to update separated by a comma',
